...
namespace moon
{
    ...
    void log::init()
    {
        ...
        log_sinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("moon.log", true));

        log_sinks[0]->set_pattern("%^[%T] %n: %v%$");
        log_sinks[1]->set_pattern("[%T] [%l] %n: %v");

        s_core_logger = std::make_shared<spdlog::logger>("MOON", begin(log_sinks), end(log_sinks));
