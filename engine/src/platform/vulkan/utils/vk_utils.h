...
{
    [[nodiscard]] inline vk::Result set_debug_object_name(vk::Device device, vk::ObjectType type, uint64_t object, const char* name)
    {
        if (!name || !*name)
    ...
    [[nodiscard]] inline vk::Semaphore create_semaphore(vk::Device device, const char* debug_name = nullptr)
    {
    ...
    [[nodiscard]] inline vk::Semaphore create_timeline_semaphore(vk::Device device, uint64_t initial_value, const char* debug_name = nullptr)
    {
    ...
    [[nodiscard]] inline vk::Fence create_fence(vk::Device device, const char* debug_name = nullptr)
    {
    ...
    [[nodiscard]] inline uint32_t find_queue_family_index(vk::PhysicalDevice physical_device, vk::QueueFlags flags)
    {
    ...
    [[nodiscard]] inline constexpr vk::Filter sampler_filter_to_vk_filter(SamplerFilter filter)
    {
        switch (filter)
        {
    ...
    [[nodiscard]] inline constexpr vk::SamplerMipmapMode sampler_mipmap_to_vk_sampler_mipmap_mode(SamplerMip filter)
    {
        switch (filter)
        {
    ...
    [[nodiscard]] inline constexpr vk::SamplerAddressMode sampler_wrap_mode_to_vk_sampler_address_mode(SamplerWrap mode)
    {
        switch (mode)
        {
    ...
    [[nodiscard]] inline constexpr vk::CompareOp compare_op_to_vk_compare_op(CompareOp op)
    {
        switch (op)
        {
    ...
    [[nodiscard]] inline vk::SamplerCreateInfo sampler_state_desc_to_vk_sampler_create_info(const sampler_state_desc& desc,
    ...
    [[nodiscard]] inline vk::SpecializationInfo get_pipeline_shader_stage_specialization_info(specialization_constant_desc desc,
    ...
    [[nodiscard]] constexpr inline stage_access get_pipeline_stage_access(vk::ImageLayout layout)
    {
        switch (layout)
        {
    ...
    inline void image_memory_barrier_2(vk::CommandBuffer cmd, vk::Image image, stage_access src, stage_access dst,
